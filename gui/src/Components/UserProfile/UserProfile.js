...
import { getToken, getUrlParams, getOwnUser, getUserPlanForms, getPlanRecordsByUser, dateToYearMonthDay, randomNumber } from '../../Functions/FormEditorFunctions';
...
class userProfile extends Component {
	...
	componentDidMount(){
		...
		let userPromise = new Promise( ( resolve, reject ) => { getOwnUser(resolve, reject); });
		userPromise.then( ( res ) => {
			...
			formsPromise.then( ( formsRes ) => {
				...
				let PlanRecordsPromise = new Promise( ( resolve, reject ) => { getPlanRecordsByUser( res.type._id, res._id, resolve, reject); });
	...
	formSelected = ( planId, planFormItemId, formId ) => {
		...
		this.setState({
			redirect: '/formRecordsByUser?planId=' + planId + '&planFormItemId=' + planFormItemId + 
						'&formId=' + formId
		});
